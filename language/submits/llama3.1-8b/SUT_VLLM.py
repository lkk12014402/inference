...
class SUT:
    ...
    def process_queries(self):
        ...
        while True:
            ...
            """
            input_ids_tensor = [
                self.data_object.input_ids[q.index].tolist() for q in qitem]
            """
            input_ids_tensor = [
            ...
            inputs = TokensPrompt(prompt_token_ids=input_ids_tensor[0])
            print(inputs)
            print(self.sampling_params)
            self.model.llm_engine.add_request('0',
                            inputs,
            ...
            while self.model.llm_engine.has_unfinished_requests():
                request_outputs: list[RequestOutput] = self.model.llm_engine.step()
                for request_output in request_outputs:
                    if request_output.finished:

                        pred_output_tokens.append(list(request_output.outputs[0].token_ids))
            ...
            for i in range(len(qitem)):
                ...
                print(processed_output[i].tolist())
                print(processed_output[i].shape[0])
                ...
                response = [
                    lg.QuerySampleResponse(
                        ...
                        bi[1],
                        n_tokens)]
                ret = lg.QuerySamplesComplete(response)
                print(ret)
